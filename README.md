...
- **Some tree-construction parse errors.** `html5-parser` 0.3.0 records a stray end tag only on the spec's "any other end tag" path (`</span>`, `</b>`), and records nothing when it repairs misnested formatting elements with the adoption agency algorithm. So a stray `</div>` (or `</li>`, `</h2>`, …) and misnested `<b><i>…</b></i>` produce no finding, although vnu reports both. The parser drops these tokens without a trace in the tree, so `html-conform` cannot detect them afterwards; the fix belongs in `html5-parser` (record the parse error at the remaining §13.2.6.4.7 "in body" branches and adoption agency steps, release it, bump the `=0.3.0` pin here).
